...
| 항목 | 방식 | 근거 |
|------|------|------|
| **API 부하 분산** | Round-robin (Nginx/ALB) | 무상태 API 서버 |
| **큐 소비 분산** | Redis Consumer Group | 각 노드가 서로 다른 메시지를 소비, 중복 방지 |
| **토픽 배정** | Consistent Hashing | 특정 토픽은 특정 노드에 배정 → 팬아웃 중복 방지 |
| **스케일링** | 수평 (노드 추가) | Docker/K8s 기반, 부하 시 노드 추가 |
| **장애 대응** | Health check + 자동 재배정 | 노드 다운 시 해당 노드의 Consumer가 소비하던 메시지를 다른 노드가 인계 (Redis XCLAIM) |
...
| 배치 멀티캐스트 | 500건 단위 청킹, 노드별 병렬 전송 |
| 팬아웃 관리 | 동시 팬아웃 수 모니터링, 초과 시 큐잉 (FCM 한도: 1,000 동시 팬아웃) |
| 메시지 큐 | Redis Streams — Consumer Group 기반 노드 간 분산 |
| 대규모 전송 | 10만건+ 요청 시 Redis에 청킹하여 적재 → 각 노드가 병렬 소비/전송 |
