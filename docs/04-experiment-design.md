...
| **제한** | 실제 100만 구독자 테스트는 비용상 불가 |
| **대안 (1)** | 가용 기기 5대를 5개 토픽에 교차 구독 → 5개 토픽 동시 전송으로 팬아웃 경쟁 유발 |
| **대안 (2)** | Firebase Admin SDK `subscribeToTopic()`으로 가상 토큰 1,000~10,000개 구독 등록 후 토픽 전송 → API 응답 시간과 FCM Data API 지표로 팬아웃 지연 간접 측정 |
| **측정 지표** | M2 (팬아웃 완료 시간), FCM Data API의 Pending 비율 |
